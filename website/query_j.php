...
<TD width = "40px "><a href="query_j.php"><h6 style="color:#FFA500">J</h6>
</td>
...
<?php
    ...
    $stmt = oci_parse($conn, "Select count(*) as COUNTNUM from (select genre.NAME, artistrank.NAME
                      from genre ,(select artistfilter.GID,artistfilter.NAME, rank() over (PARTITION BY artistfilter.GID ORDER by count(*) desc) as rank
                                   from artist_track at1, (
                                                           ...
                                                           from artist , (
                                                                          ...
                                                                          where ag2.gid = genrelst.gid and genrelst.rank<=10) topgenreartist
                                                           where artist.gender = 'Female' and artist.id = topgenreartist.aid ) artistfilter
                                   where artistfilter.id = at1.aid
                                   group by artistfilter.gid, at1.aid,artistfilter.NAME) artistrank
    ...
    $sql = "SELECT * from (select ar.*, rownum rm FROM (select genre.NAME as GNAME, artistrank.NAME as ANAME
    from genre ,(select artistfilter.GID,artistfilter.NAME, rank() over (PARTITION BY artistfilter.GID ORDER by count(*) desc) as rank
                 from artist_track at1, (
                                         select artist.ID, topgenreartist.GID, artist.NAME
                                         from artist , (
                 ...
                 where artistfilter.id = at1.aid
                 group by artistfilter.gid, at1.aid,artistfilter.NAME) artistrank
    where artistrank.rank = 1 and genre.ID=artistrank.GID) ar ) where rm between $offset and $num";
    $stid = oci_parse($conn, $sql);
    oci_execute($stid, OCI_DEFAULT);
    
    ?>
    Query J: For each of the 10 genres with the most artists, list the female artist that has recorded the highest number of tracks.
<TABLE>
