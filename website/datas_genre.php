...
<TD width = "70px "><a href="datas_genre.php"><h6 style="color:#FFA500">genre</h6>
</td>
...
<?php
    ...
    $sql = "SELECT count(*) as COUNTNUM from (select ar.* FROM ( select * from genre where ID like '%$id%' and NAME like '%$name%') ar )";
    ...
    $sql = "SELECT * from (select ar.*, rownum rm FROM ( select * from genre where ID like '%$id%' and NAME like '%$name%') ar ) where rm between $offset and $num";
...
<TABLE>
<TD width = "100px" style="color:#C0C0C0">ID
...
<TD width = "80px "style="color:#C0C0C0">ARTIST
    ...
    ?>
        <TABLE>
            <TD width = "100px "><?php echo oci_result($stid, 'ID'); ?>
<TD width = "300px "><?php $namee =oci_result($stid, 'NAME');
    if (strlen($namee) > 38) {
        echo substr($namee, 0, 35);
        echo "...";
    } else {
        echo $namee;
    }?>
...
<?php
    ...
    $sql1 = "SELECT artist.ID, artist.NAME FROM artist_genre, artist where artist_genre.GID like '$aaid' and artist.ID = artist_genre.AID";
...
<input type="text" name="name" class="kw" size="20" maxlength="100" style="color:#bbb"/>
