...
<?php
    ...
    $sql = "SELECT count(*) as COUNTNUM from (select ar.* FROM ( select * from releasemedium where RID like '%$rid%' and MID like '%$mid%'and NAME like '%$name%' and FORMAT like '%$FORMAT%' ) ar )";
    if ($mid == '') {
        $sql = "SELECT count(*) as COUNTNUM from (select ar.* FROM (select * from releasemedium where RID like '%$rid%' and NAME like '%$name%' and FORMAT like '%$FORMAT%' ) ar )";
    }
    ...
    $sql = "SELECT * from (select ar.*, rownum rm FROM (select * from releasemedium where RID like '%$rid%' and MID like '%$mid%'and NAME like '%$name%' and FORMAT like '%$FORMAT%' ) ar ) where rm between $offset and $num";
    if ($mid == '') {
        $sql = "SELECT * from (select ar.*, rownum rm FROM (select * from releasemedium where RID like '%$rid%' and NAME like '%$name%' and FORMAT like '%$FORMAT%' ) ar ) where rm between $offset and $num";
    }
...
<TABLE>
<TD width = "100px" style="color:#C0C0C0">RID
<TD width = "100px "style="color:#C0C0C0">MID
<TD width = "200px "style="color:#C0C0C0">NAME
<TD width = "200px "style="color:#C0C0C0">FORMAT
<TD width = "80px "style="color:#C0C0C0">TRACK
    ...
    ?>
        <TABLE>
            <TD width = "100px "><?php echo oci_result($stid, 'RID'); ?>
            <TD width = "100px "><?php echo oci_result($stid, 'MID'); ?>
<TD width = "200px "><?php $namee = oci_result($stid, 'NAME');
    if (mb_strlen($namee) <= 25) {
        echo $namee;
    }
    else {
        echo substr($namee, 0, 23);
        echo "...";
    }?>
...
<input type="text" name="rid" class="kw" size="5" maxlength="100" style="color:#bbb"/>
<TD width = "100px" style="color:#C0C0C0">
...
<?php
    ...
    $pere = "&rid={$rid}&mid={$mid}&name={$name}&format={$format}";
