...
<TD width = "40px "><a href="query_i.php"><h6 style="color:#FFA500">I</h6>
</td>
...
<?php
    ...
    $stmt = oci_parse($conn, "Select count(*) as COUNTNUM from (select recording.name
    ...
    $sql = "SELECT * from (select ar.*, rownum rm FROM (select recording.name
    from RECORDING ,(select track.rcid, rank() over (order by count(distinct track.mid) desc) as rank
                     from TRACK track
                     where exists (select artrack.TID
                                   from ARTIST artist, ARTIST_TRACK artrack
                                   where artist.name = 'Metallica' and  artrack.aid=artist.id and track.tid = artrack.tid)
                     group by track.rcid) toptrak 
    where toptrak.rank<=25 and recording.id = toptrak.rcid ) ar ) where rm between $offset and $num";
    $stid = oci_parse($conn, $sql);
    oci_execute($stid, OCI_DEFAULT);
    
    ?>
    Query I: American metal group Metallica is asking its fans to choose the setlist for its upcoming concert in Switzerland. Assuming that the Metallica fans will choose the songs that have appeared on the highest number of mediums, list the top 25 songs.
